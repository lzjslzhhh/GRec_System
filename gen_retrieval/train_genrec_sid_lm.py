...
def read_sid_stats(train_jsonl: str, sid_stats_path: str) -> Dict[str, object]:
    path = sid_stats_path or (train_jsonl + ".stats.json")
    if not os.path.exists(path):
        raise FileNotFoundError(f"sid stats not found: {path}")
    with open(path, "r", encoding="utf-8") as f:
        stats = json.load(f)
    if "sid_vocab_size" not in stats:
        raise ValueError(f"Invalid sid stats file, missing sid_vocab_size: {path}")
    return stats
...
class SidLmTrainDataset(Dataset):
...
class SidLmEvalDataset(Dataset):
    def __init__(self, rows: List[dict], max_len: int):
        self.rows = rows
        self.max_len = max_len
    ...
    def __getitem__(self, idx: int) -> Dict[str, object]:
        ...
        history_item = [int(x) for x in ex.get("history_item", [])]
        target_item = int(ex.get("target_item", target))
        return {
            "history": history,
            "target": target,
...
def build_eval_collate(pad_id: int):
    def _fn(batch: List[Dict[str, object]]) -> EvalBatch:
        ...
        target_item: List[int] = []
        for i, ex in enumerate(batch):
            hist: List[int] = ex["history"]  # type: ignore[assignment]
            L = len(hist)
            input_ids[i, :L] = torch.tensor(hist, dtype=torch.long)
            attn[i, :L] = True
            target[i] = int(ex["target"])
...
def main():
    ...
    ap.add_argument("--sid_stats_path", type=str, default="")
    ap.add_argument("--kmeans_k", type=int, default=512, help="for tracking only")
    ...
    sid_stats = read_sid_stats(args.train_jsonl, args.sid_stats_path)
    token_vocab_size = int(sid_stats["sid_vocab_size"])
    unk_sid = sid_stats.get("unk_sid", None)
    pad_id = token_vocab_size
    model_vocab_size = token_vocab_size + 1
    if model_vocab_size <= 2:
        raise ValueError(f"Invalid SID vocab size: {token_vocab_size}")

    train_rows = load_jsonl(args.train_jsonl)
    val_rows = load_jsonl(args.val_jsonl)
    train_ds = SidLmTrainDataset(train_rows, max_len=args.max_len)
    val_ds = SidLmEvalDataset(val_rows, max_len=args.max_len)
    ...
    config = {
        ...
        "unk_id": int(unk_sid) if unk_sid is not None else None,
        "pad_id": int(pad_id),
        ...
        "sid_stats_path": args.sid_stats_path or (args.train_jsonl + ".stats.json"),
    }
    ...
    print(
        ...
        f"token_vocab_size={token_vocab_size} model_vocab_size={model_vocab_size}",
        flush=True,
    )
